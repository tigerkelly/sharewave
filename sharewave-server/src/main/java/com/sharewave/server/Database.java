...
public class Database {
     ...
     */
    /** Records the current time as the last-downloaded timestamp for a file. */
    public void updateLastDownloaded(int fileId) throws SQLException {
        try (PreparedStatement ps = conn.prepareStatement(
                "UPDATE files SET last_downloaded=strftime('%s','now') WHERE id=?")) {
            ps.setInt(1, fileId);
            ps.executeUpdate();
        }
    }
    ...
    /** Builds a FileRecord from a ResultSet that includes the standard file columns. */
    private FileRecord fromResultSet(ResultSet rs) throws SQLException {
